...
// Khai bao bien status
uint8_t status_lm35 = 0;

// Khai bao bien setup thoi gian gan nhat
...
void Display_Warning()
{
    ...
    case 2:
        ...
        Lcd_write_string("====================");
        Lcd_gotoxy(5, 1);
        Lcd_write_string("TROI DA SANG!");
        Lcd_gotoxy(0, 2);
        Lcd_write_string("TAT DEN LED");
        Lcd_gotoxy(0, 3);
        Lcd_write_string("====================");
        display = 1;
        break;
    case 3:
        ...
        Lcd_write_string("====================");
        Lcd_gotoxy(5, 1);
        Lcd_write_string("TROI DA TOI!");
        Lcd_gotoxy(0, 2);
        Lcd_write_string("BAT DEN LED");
        Lcd_gotoxy(0, 3);
        Lcd_write_string("====================");
        display = 1;
        break;
...
void LED_Effect()
{
    if (HAL_GetTick() - led_toggle_time >= 5)
    {
...
void Buzzer_Effect()
{
    if (HAL_GetTick() - buzzer_toggle_time >= 5)
    {
...
int main(void)
{
    ...
    while (1)
    {
        ...
        if (status_lm35 == 1)
        {
            ...
            Mode_Save = 1;
            Save_Data();
            status_lm35_time = HAL_GetTick(); // Lay thoi diem start warning
        }

        if (Mode_Save == 1)
        {
            if (HAL_GetTick() - status_lm35_time >= 1000)
            {
                display = 0;
                Mode_Save = 0;
                status_lm35 = 0;
            ...
            {
                Save_Data(); // Giu hien thi canh bao
            }
            continue;
        }
...
static void MX_GPIO_Init(void)
{
    ...
    /*Configure GPIO pin Output Level */
    HAL_GPIO_WritePin(GPIOB, GPIO_PIN_0 | GPIO_PIN_1 | GPIO_PIN_2 | GPIO_PIN_10 | GPIO_PIN_12 | GPIO_PIN_13 | GPIO_PIN_14 | GPIO_PIN_15 | GPIO_PIN_3 | GPIO_PIN_4 | GPIO_PIN_5 | GPIO_PIN_6 | GPIO_PIN_7 | GPIO_PIN_8 | GPIO_PIN_9, GPIO_PIN_RESET);
